...
def run_NNetwork(net, post = False):
    #starting values
    route=[]
    route.append("HUB")
    genome_fitness = 0
...
def change_values():
    try:
        MAX_PACKAGES = MAX_PACKAGES_INT.get()
    except:
        print("missing int")
    print("Complete")
...
title = customtkinter.CTkLabel(app, text= "Insert Max Packages")
title.pack(padx=10, pady=10) 


MAX_PACKAGES_INT = tkinter.IntVar()
package_request = customtkinter.CTkEntry(app, width=40, height=40, textvariable=MAX_PACKAGES_INT)
package_request.pack()
Summit_Values = customtkinter.CTkButton(app,text="confirm", command=change_values)
Summit_Values.pack()    
